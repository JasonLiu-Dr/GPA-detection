...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes):
        ...
        if self.training:
            ...
            rois, rpn_loss_cls, rpn_loss_bbox, rpn_cls_prob, rois_select = self.RCNN_rpn(base_feat, im_info, gt_boxes, num_boxes)
            tgt_rois, tgt_rpn_loss_cls, tgt_rpn_loss_bbox, tgt_rpn_cls_prob, tgt_rois_select = self.RCNN_rpn(tgt_base_feat,
        ...
        if cfg.POOLING_MODE == 'crop':
            ...
            if self.training:
                grid_xy_ = _affine_grid_gen(rois_select, base_feat.size()[2:], self.grid_size)
                grid_yx_ = torch.stack([grid_xy_.data[:, :, :, 1], grid_xy_.data[:, :, :, 0]], 3).contiguous()
                pooled_feat_ = self.RCNN_roi_crop(base_feat, Variable(grid_yx_).detach())
                if cfg.CROP_RESIZE_WITH_MAX_POOL:
                    pooled_feat_ = F.max_pool2d(pooled_feat_, 2, 2)

                tgt_grid_xy_ = _affine_grid_gen(tgt_rois_select, tgt_base_feat.size()[2:], self.grid_size)
                tgt_grid_yx_ = torch.stack([tgt_grid_xy_.data[:, :, :, 1], tgt_grid_xy_.data[:, :, :, 0]], 3).contiguous()
                tgt_pooled_feat_ = self.RCNN_roi_crop(tgt_base_feat, Variable(tgt_grid_yx_).detach())
                if cfg.CROP_RESIZE_WITH_MAX_POOL:
            ...
            if self.training:
                ...
                tgt_pooled_feat_ = self.RCNN_roi_align(tgt_base_feat, tgt_rois_select)
            ...
            if self.training:
                ...
                tgt_pooled_feat_ = self.RCNN_roi_pool(tgt_base_feat, tgt_rois_select)
        ...
        if self.training:
            ...
            if self.rpn_mode == 'adapt':
                RPN_loss_intra, RPN_loss_inter = self.adaptive_loss_rpn(rpn_adapt_feat, rpn_cls_prob, tgt_rpn_adapt_feat,
    ...
    def adaptive_loss_rpn(self, pooled_feat, cls_prob, tgt_pooled_feat, tgt_cls_prob, batch_size, epsilon=1e-6):
    ...
    def gcn_adaptive_loss(self, pooled_feat, cls_prob, rois, tgt_pooled_feat, tgt_cls_prob, tgt_rois, batch_size, epsilon = 1e-6):
        ...
        for i in range(num_classes):
            ...
            for j in range(batch_size):
                tmp_batch_feat = tmp_class_feat[j, :, :]
                tmp_batch_adj = get_adj(rois[j, :, :])

                # graph-based aggregation
                tmp_batch_feat = torch.mm(tmp_batch_adj, tmp_batch_feat)
                tmp_batch_weight = tmp_cls_prob[j, :, :]
                tmp_batch_weight = torch.mm(tmp_batch_adj, tmp_batch_weight)
            ...
            for j in range(batch_size):
                tmp_tgt_batch_feat = tmp_tgt_class_feat[j, :, :]
                tmp_tgt_batch_adj = get_adj(tgt_rois[j, :, :])

                # graph-based aggregation
                tmp_tgt_batch_feat = torch.mm(tmp_tgt_batch_adj, tmp_tgt_batch_feat)
                tmp_tgt_batch_weight = tmp_tgt_cls_prob[j, :, :]
                tmp_tgt_batch_weight = torch.mm(tmp_tgt_batch_adj, tmp_tgt_batch_weight)
